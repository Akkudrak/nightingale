...
- 🛍️ **Online catalog importer** — a separate slim Tauri 2 mini-app (`client-catalog-importer/`) installs alongside Nightingale and accepts catalog songs through a dedicated `nightingale-import://` deep-link scheme. Drops catalog ZIPs straight into your existing library (songs.db + cache/), runs alongside Nightingale 1.0.0+, and shares zero UI with the main client. The first-run wizard asks for your system folder and library folder once, then handles every catalog URL you throw at it. See [docs/catalog-importer](site/docs/src/catalog-importer.md).
...
🛍️ **Online catalog imports** — drop a `nightingale-import://…` deep link onto your Nightingale install and a tiny standalone importer (`client-catalog-importer/`) fetches the catalog ZIP, validates it, hashes the MP3 with the same blake3 prefix the rest of the app uses, drops the cover into `cache/<hash>_cover.jpg`, renames the MP3 to `<slug título>-<artista>-<XXXXXX>.mp3`, and writes a row into the existing `songs.db`. Designed for both Nightingale 1.3+ (which keeps using `nightingale://`) and older versions that ship without deep-link routing — the importer is its own Tauri 2 binary with five capabilities (no opener, no process, no updater, no assetProtocol) and only touches `songs.db` + `cache/` under the folder you pick during the first-run wizard. See [docs/catalog-importer](site/docs/src/catalog-importer.md).
