...
- "Recent plays" view in the Leaderboards dialog. A new entry in the sidebar menu opens a table with the last 20 scoring attempts across every profile and song, sorted newest first. Each row shows the date, song title/artist, profile, score, and a clickable mic icon that plays the matching recording inline (or `—` when no take was saved). The same composite key (`profile` + `song_hash` + `score` + `played_at`) that `usePlaybackResult` writes in a single snapshot joins a score row to its recording deterministically — see `recordingJoinKey` in `client/src/features/profiles/components/leaderboards.tsx`. The shared `RecordingPlayer` was extracted from `RecordingsDialog` so both surfaces render the same inline player without duplicating audio wiring.
- Per-song audio preview. Every row in the desktop song list (table view and grid card) now has a play/pause button that opens a global preview bar mounted in `MenuLayout`. The preview plays the instrumental track with the vocal stem mixed in at 0.55 when stems are available, falls back to the original audio for songs without stems, and synchronizes the two `<audio>` elements on play/pause/seek via the same drift-resync pattern the guest preview uses. A small module-scoped LRU keyed by song hash avoids re-running `playbackAdapter.getAudioPaths` on every click. The preview bar stops itself the moment the karaoke route opens (`useStopPreviewOnPlayback` watches `useLocation().pathname.startsWith('/playback')`) so the karaoke audio engine owns the output device without a stutter.
- Microphone recordings are now persisted to `app-core::recording` with their `played_at` timestamp and exposed via three Tauri commands (`load_recordings`, `save_recording`, `delete_recording`) plus a `get_recording_path` helper. The new `RecordingStore` is keyed by recording id and shares `played_at` with `ScoreRecord` so the two stores can be joined without heuristics. The `RecordingRecord` and `RecordingStore` TypeScript bindings are regenerated via ts-rs (`cargo test -p app-core --lib export_bindings`).
- Tauri window no longer needs a JS-side handshake to appear. `WebviewWindowBuilder::from_config` does not propagate `visible` when `tauri.conf.json` sets `create: false`, so the window used to stay invisible if `triggerFrontendReady()` never ran (any render error before `TauriAppShell` mounted would leave the process alive with zero visible windows). The builder now calls `.visible(true)` explicitly so the window appears at startup regardless of JS state; the existing `frontend_ready` command remains as a no-op for older builds. See `client/src-tauri/src/lib.rs` setup hook.
- Per-song leaderboard now lists every scoring attempt for a song instead of only the best score per profile. The branch in `LeaderboardBoard` for `song-leaderboard` mode shows date, profile, score, and a clickable mic icon — same columns as the Recent plays view — so a player can see how they have improved over time across multiple takes of the same song. The dialog description and the action-item description in the song details panel were updated to reflect the new behaviour.
- Standalone catalog importer for Nightingale 1.0.0+. The new `client-catalog-importer/` Tauri 2 mini-app installs alongside older Nightingale versions and lets them receive catalog songs through a dedicated `nightingale-import://` deep-link scheme (the existing `nightingale://` scheme on Nightingale 1.3+ is untouched). On first run, a wizard asks for the existing Nightingale system folder (parent of `songs.db` / `cache/`) and the user's music library folder; the choices are persisted to `%APPDATA%/com.rzru.catalog-importer/config.json`. Subsequent deep-link payloads download the catalog ZIP to a temp file under the system folder, then `app-core`'s new `catalog_import::import_catalog_zip` validates the structure, hashes the MP3 with the same blake3 prefix the rest of the app uses, drops the cover into `cache/<hash>_cover.jpg` (allow-listed hosts only), renames the MP3 to `<slug título>-<artista>-<XXXXXX>.mp3`, and writes a row into the existing `songs.db` via `upsert_song_compat` (probes `PRAGMA table_info(songs)` and builds a dynamic `INSERT` so v1 / v2 / v3 schemas all work). Schema migrations are intentionally skipped on the importer side via a new `MigrateMode::ProbeOnly` so the importer never alters the user's DB behind their back — `genre` and `added_at` only appear when the main Nightingale app itself migrates the connection. Concurrent main-app writes are absorbed by a 3-attempt retry (250 / 500 ms backoff) on `SQLITE_BUSY` / `SQLITE_LOCKED`. The whole flow shares zero logic with the main client Tauri shell (only the `app-core` primitives `slug_root_folder`, `blake3_short_hex`, `CacheDir::cover_path`, and `importer_config`); the React UI is intentionally a thin two-step wizard + status view and uses the same `tauri-plugin-dialog` folder picker the rest of the app uses. Capabilities for the new mini-app are exactly five: `core:default`, `core:window:allow-set-focus`, `core:window:allow-show`, `dialog:default`, `deep-link:default` — no `opener`, no `process`, no `updater`, no `assetProtocol`. CSP is `default-src 'self' ipc: http://ipc.localhost`. The `ureq` agent uses `native-tls` (mirroring the YouTube change below) so downloads survive Avast-MITM proxies on Windows hosts. The importer only touches `songs.db` and `cache/` under the user-picked system folder; it never reads or writes `~/.nightingale/config.json`.

### Fixes

- Desktop window content went blank instead of rendering the menu after a recent change. `useStopPreviewOnPlayback` was mounted in `MenuLayout` and read `usePlaybackTransportState`, but `PlaybackTransportProvider` only wraps the `/playback` route — the menu tree sits outside the provider, so the hook threw `usePlaybackTransportState must be used within a PlaybackTransportProvider` and React died before mounting. The hook now watches `useLocation().pathname.startsWith('/playback')` instead, which is the actual signal that the karaoke engine is about to take over the audio device.
...
- Queue "Added by" now matches the actual queuer in server/guest mode. `useProfileMutations` flips the cached `active` synchronously in `onMutate` (with rollback in `onError`) and `useGuestProfile`'s reconciliation effect routes through `mutateAsync` instead of calling `switchProfile` directly. Previously, a guest who reloaded `/guest` with a stored profile could queue a song during the `switch_profile` IPC window — `useCurrentProfile()` still read the previous active from the cache, so the entry's `addedBy` was frozen to the old user while the score (which reads `ProfileStore.active` on the server at IPC arrival) landed under the right one.
